exports.run = async (client, msg, args) => {
 ...
 description: (':no_entry_sign: Ya geçerli birini etiketle ya da sana sövmemi istiyosan kendini etiketle.')
  ...
  if(member.id === "416271180945686528")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (':no_entry_sign: Sen kimsin kardeşime sövüyorsun amına koduğumun jedayı!')
}})
  if(member.id === "478943598151335974")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop! Yapımcımı ne etiketliyorsun orospu çocuğu?!`)
}})
  if(member.id === "450289701233098763")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop! Ortağıma sövemezsin ikile kardeşim.`)
}})
